...
\toprule
\textbf{Role} & \textbf{Name} & \textbf{Task description and etc.} \\
\midrule
...
\toprule
\textbf{Directory} & \textbf{Main files} \\
\midrule
...
\subsection{Initial Functional Test Case Table (Excerpt)}

Table~\ref{tab:testcases} shows an excerpt of the functional test cases.  
(The full table in the original documentation is longer; only representative entries are shown here to fit the page limit.)

\begin{table*}[!t]
\centering
\caption{Initial functional test cases (excerpt)}
\label{tab:testcases}
\renewcommand{\arraystretch}{1.1}
...
\toprule
\textbf{Use Case} & \textbf{Function tested} & \textbf{Initial state} & \textbf{Input} & \textbf{Expected output} \\
\midrule
User Login (valid) &
System authenticates user and opens main dashboard &
User is on Login Screen, not authenticated &
User enters valid email and password, then clicks login &
User is authenticated and Screen~2 (Main Dashboard) is displayed. \\
\midrule
User Login (invalid) &
System rejects invalid credentials &
User is on Login Screen, not authenticated &
User enters invalid email or password, then clicks login &
...
System navigates to Screen~3 (Presentation Preparation Screen). \\
\midrule
Start Meeting Immediately &
Routing to Screen~5 &
User authenticated, on Screen~2 &
Click ``Start Meeting'' &
System navigates to Screen~5 with an empty agenda map. \\
\midrule
Start Teleprompter &
STT and highlighting start &
Screen~4 loaded, STT off &
Click ``Start'' &
STT is activated and real-time highlighting begins. \\
\midrule
End Meeting &
Summary generation and navigation &
Meeting active, Screen~5 shown &
Click ``End Meeting'' &
System navigates to Screen~6 (Meeting Summary Report) and displays the final agenda map and timelines. \\
\bottomrule
\end{tabularx}
\end{table*}
